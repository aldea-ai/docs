...
client := dglisten.NewREST("", &dgi.ClientOptions{
    Host:        "https://api.subquadratic.ai",
    AccessToken: "YOUR_SUBQ_API_KEY",
})
```
...
Run a quick test to confirm the migration. These examples use the same SDK methods you already have, just with the SubQ configuration from Step 2.
