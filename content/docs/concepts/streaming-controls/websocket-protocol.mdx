...
The SubQ streaming API uses WebSocket for bidirectional communication. Your client sends binary audio frames and JSON control messages; the server responds with JSON transcript messages.
...
| `wss://api.subquadratic.ai/v1/listen/pcm` | Raw PCM only (optimized) |
